...
export default function WriteDownNote({ navigation }) {
  ...
  const [DeleteTasteId, setDeleteTasteId] = useState({vId: 0, bId: 0});
  ...
  const addTaste = () => {
    ...
    const newTaste = {
      ...
      id: TasteId,
      text: buttonText,
      textLen: Math.floor(textInputSize.width),
    }; //key겹침 해결해야됨
    ...
    여섯글자 : 96 * 3
    */
    if (buttonTitleLen > 264 || TasteViews.length === 0) {
  ...
  const DeleteTasteDialog = (vId, bId) => {
    //느낌 버튼 삭제 Dialog on/off
    setDeleteTasteId({vId, bId});
    setDeleteTaste(!DeleteTaste);
  };

  const DeleteTasteBtn = () => {//느낌 버튼 삭제하기
    const updatedTastes = Tastes[DeleteTasteId.vId].filter(taste => taste.id !== DeleteTasteId.bId);
    Tastes[DeleteTasteId.vId]=updatedTastes;
    if(updatedTastes.length === 0){//느낌 버튼 한줄이 없어졌을 때
      console.log(DeleteTasteId.vId);
      const updatedTasteViews = TasteViews.filter(view => view.id !== DeleteTasteId.vId);
      setTasteViews(updatedTasteViews);
    }
    setDeleteTaste(!DeleteTaste);
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
                    ...
                    {Tastes[views.id].map((button) => (
                      <View key={button.id} style={{ marginRight: 5 }}>
                        <TouchableOpacity
                          ...
                          onLongPress={() => DeleteTasteDialog(views.id, button.id)}
                          style={{
                            marginRight: 4,
                            backgroundColor: pressedButtonIds.includes(
                              button.id
                            )
                              ? "rgb(230,230,230)"
                        ...
                        >
                          <Text
                            style={{ fontSize: 20, color: "rgb(80,80,80)" }}
                          >
                            {button.text}
                          </Text>
                        </TouchableOpacity>
      ...
      {/* 버튼 삭제 Dialog */}
      <Dialog isVisible={DeleteTaste} onBackdropPress={() => DeleteTasteDialog(0, 0)}>
        ...
        <Dialog.Actions>
          ...
          >
            ...
            />
            <Dialog.Button title="취소" onPress={() => DeleteTasteDialog(0, 0)} />
          </View>
        </Dialog.Actions>
      </Dialog>
    </SafeAreaView>
